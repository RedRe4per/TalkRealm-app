...
function useUpdateUsers(
  ...
  useEffect(() => {
    for (let remoteUser of remoteUsers) {
      if (
        userList.findIndex(
          (user: any) => user.userPeerId === remoteUser?.userPeerId
        ) < 0
      ) {
    ...
    for (let user of userList) {
      ...
      const index = remoteUsers.findIndex(
        (remoteUser: any) => remoteUser?.userPeerId === user.userPeerId
      );
      if (index < 0) {
        const emptyIndex = remoteUsers.findIndex(
          (remoteUser: any) => remoteUser === null
        );
